...
class FacebookPage:
    ...
    def post(self, text):
        self.graph.put_object(self.page_id, "feed", message=text)

    @staticmethod
    def generate_post(update: Update) -> str:
        ...
        message += f"{website}/miui/{short_codename}\n\n"
    ...
    def post_updates(self, new_updates: List[Update]):
        ...
        for update in new_updates:
            text = self.generate_post(update)
            self.post(text)
