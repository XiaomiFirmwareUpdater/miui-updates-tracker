...
cat dl_links | sed -n '/none/!p' | while read line; do
	...
	python telegram.py -t $bottoken -c @MIUIUpdatesTracker -D -M "New weekly update available!
	*Device*: $name
	*Product*: $model
	*Codename*: $codename
	*Version*: $version
...
#Push
git add weekly_db ; git -c "user.name=$gituser" -c "user.email=$gitmail" commit -m "Sync: $(date +%d.%m.%Y-%R)"
